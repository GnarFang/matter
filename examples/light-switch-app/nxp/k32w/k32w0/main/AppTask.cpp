...
static LEDWidget sLightSwitchLED;
#endif
...
// Initialize device attestation config
    ...
    sLightSwitchLED.Init(LIGHT_SWITCH_STATE_LED);
#endif
...
#if !cPWR_UsePowerDownMode
        if (sAppTask.mFunction != kFunction_FactoryReset && sAppTask.mFunction != kFunction_Identify)
        {
...
{
    if ((pin_no != RESET_BUTTON) && (pin_no != LIGHT_SWITCH_BUTTON) && (pin_no != OTA_BUTTON) && (pin_no != BLE_BUTTON))
    {
        return;
    }
...
void AppTask::HandleKeyboard(void)
{
    ...
    while (eventMask)
    {
        ...
        case gKBD_EventReleasePB1_c:
            K32W_LOG("gKBD_EventReleasePB1_c");
...
void AppTask::ResetActionEventHandler(AppEvent * aEvent)
{
    ...
    else
    {
        ...
        sLightSwitchLED.Set(false);

        sStatusLED.Blink(500);
        sLightSwitchLED.Blink(500);
#endif
...
void AppTask::DimmerActionEventHandler(AppEvent * aEvent)
{
}
