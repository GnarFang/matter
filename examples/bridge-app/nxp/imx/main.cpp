...
// vector<mac[LEN_MAC_NIBBLE+2]> BridgeNodesMac;
newdb_zcb_t ZcbLight1Node;
// newdb_zcb_t BridgeNode;
...
void RegisterDevice()
{
    ...
    // // Get ActionLight1 zcb node
    // newDbGetOneZcbNode(&ZcbLight1Node);
    // printf("LED1 Saddr %x", ZcbLight1Node.saddr);
}
...
void * bridge_polling_thread(void * context)
{
    while (1)
    {
        if (kbhit())
        {
            ...
            {
                newDbGetOneZcbNode(&ZcbLight1Node);
                printf("updateLED1 Saddr %x", ZcbLight1Node.saddr);
            }
...
int main(int argc, char * argv[])
{
    ...
    // Add 4 lights for the Action Clusters tests
    newDbGetOneZcbNode(&ZcbLight1Node);
    printf("==== matter brige get LED1 Saddr %x from ZCB \n", ZcbLight1Node.saddr);
    AddDeviceEndpoint(&ActionLight1, &bridgedLightEndpoint, Span<const EmberAfDeviceType>(gBridgedOnOffDeviceTypes),
                      Span<DataVersion>(gActionLight1DataVersions), 1);
