...
{
    Action *actions[action_nums];

    actions[0] = new Action(0x1001, "permit join", Actions::ActionTypeEnum::kAutomation, 0xE001, 0x1, Actions::ActionStateEnum::kInactive, true);
    actions[1] = new Action(0x1002, "bridge reset", Actions::ActionTypeEnum::kAutomation, 0xE001, 0x1, Actions::ActionStateEnum::kInactive, true);

    for(int i = 0; i < action_nums; i++ ) {
...
int BridgeActions::handle(uint16_t actionID, EndpointId endpointID, uint32_t invokeID, bool hasInvokeID)
{
